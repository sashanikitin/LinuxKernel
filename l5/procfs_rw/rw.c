...
#define PROC_DIRECTORY  "example_time"
#define PROC_FILENAME   "last"
...
static u64 last_read_time=0;
static int example_read(struct file *file_p, char __user *buffer, size_t length, loff_t *offset)
{	
	static u64 last_read_time=0;
	u64 time = get_jiffies_64();
	if(last_read_time==0){
		printk("You haven't read yet");       
	}else{
		printk("%u msec has passed\n", time-last_read_time);
	}
	last_read_time = time;
	return 0; 
...
static int __init example_init(void)
{
	...
	proc_msg_length = strlen(proc_buffer);
